...
func TestForm_MinLength(t *testing.T) {
	...
	form.MinLength("Stephen", 5)
	if form.Valid() {
		t.Error("form shows minLength for non existent field")
	}

	isError := form.Errors.Get("Stephen")
	if isError == "" {
		t.Error("should have an error, but did not get one")
	}

	postedValues := url.Values{}
	postedValues.Add("stephen", "montague")
	form = New(postedValues)

	form.MinLength("stephen", 100)
	if form.Valid() {
		t.Error("shows min length of 100 met when data is shorter")
	}

	postedValues = url.Values{}
	postedValues.Add("montague", "stephen")
	form = New(postedValues)

	form.MinLength("montague", 1)

	if !form.Valid() {
		t.Error("shows min length of 1 is not met when it is")
	}

	isError = form.Errors.Get("montague")
	if isError != "" {
